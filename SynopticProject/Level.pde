    ...
    for(int i = 0; i < cbArray.size(); i++){
      if(cbArray.get(i).getCBText().toLowerCase().equals("input")){
        if(levelInput.size() == 0){
          failMessage = "You have failed! You can not input from an empty array! \n Please reset the level to try again!";
          hasFailed = true;
        } else{
        ...
        if(storedValue == 404){
          failMessage = "You have failed! You do not have a value stored! \n Please reset the level to try again!";
          hasFailed = true;
        } else {
        ...
        if(storedValue == 404){
          failMessage = "You have failed! You do not have a stored value to compare to for your if block! \n Please reset the level to try again!";
          hasFailed = true;
    ...
    if(initialArray == true){
      if(levelOutput.size() == finalLevelOutput.size()){
        ...
        if(arraysEqual == false){
          failMessage = "You have failed! Your output does not match the expected output! \n Please reset the level to try again!"; 
          hasFailed = true;
        } else{        
          success();
        }
